...
let add_map_stmt (stmt : 'a statement) m =
  ...
  in
  let do_x stmt_loc m expr = List.fold_left ~f:(map stmt_loc) ~init:m (expr_locs expr) in
  let do_xs stmt_loc m exprs = List.fold_left ~f:(do_x stmt_loc) ~init:m exprs in
...
let search (sigma : 'a sigma) =
  List.fold_right ~f:(fun (_, (_, _, _, _, stmt)) m -> add_map_stmt stmt m)
    sigma.function_definitions ~init:LocMap.empty
