...
and free_vars_list : 'a term list -> SymSet.t = 
  fun xs ->
  List.fold_left ~f:(fun ss t ->
      SymSet.union ss (free_vars t)
    ) ~init:SymSet.empty xs
...
let rec subst (su : [`Term of typed | `Rename of Sym.t] subst) (IT (it, bt, loc)) =
  match it with
  | Sym sym ->
     begin match List.Assoc.find su.replace ~equal:Sym.equal sym with
     | Some (`Term after) ->
  ...
  | Struct (tag, members) ->
     IT (Struct (tag, List.map ~f:(Pair.map_snd ~f:(subst su)) members), bt, loc)
  | StructMember (t, m) ->
     IT (StructMember (subst su t, m), bt, loc)
  | StructUpdate ((t, m), v) ->
     IT (StructUpdate ((subst su t, m), subst su v), bt, loc)
  | Record members ->
     IT (Record (List.map ~f:(Pair.map_snd ~f:(subst su)) members), bt, loc)
  | RecordMember (t, m) ->
...
and suitably_alpha_rename_pattern su (Pat (pat_, bt, loc), body) =
  ...
  | PConstructor (s, args) ->
     let body, args =
       Stdlib.List.fold_left_map (fun body (id, pat') ->
           let pat', body = suitably_alpha_rename_pattern su (pat', body) in
           (body, (id, pat'))
         ) body args
     in
...
let vargs_binop basevalue binop = function
  | [] -> basevalue
  | it::its -> List.fold_left ~f:binop ~init:it its
...
let (%.) struct_decls t member =
  ...
  in
  let member_bt =
    let member_types = Memory.member_types (SymMap.find tag struct_decls) in
    match List.Assoc.find member_types ~equal:Id.equal member with
...
let record_ members loc =
  IT (Record members,
      BT.Record (List.map ~f:(Pair.map_snd ~f:basetype) members), loc)
