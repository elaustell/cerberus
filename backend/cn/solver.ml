...
module Translate = struct
  ...
  let translate_datatypes context global =
    ...
    let translate_group to_translate =
        ...
        | BT.Datatype nm -> begin match BT_Table.find_opt bt_table bt with
            ...
            | None ->
                (* if nm is not in BT_Table, then nm must be in the current
                   ...
                   would be different *)
                (None, List.Assoc.find_exn to_translate ~equal:Sym.equal nm)
          end
      ...
      in
      List.iter2_exn ~f:(fun (nm, _) sort -> begin
  ...
  let term ?(warn_lambda=true) context global : IT.t -> expr =
    ...
    let apply_matching_func sym_role fds args =
      ...
      in
      let fd = List.find_exn ~f:matching fds in
    ...
    let rec term it =
      ...
      | RecordMember (t, member) ->
         ...
         let n = List.Assoc.find_exn members_i ~equal:Id.equal member in
         let destructors = Z3.Tuple.get_field_decls (sort (IT.bt t)) in
         Z3.Expr.mk_app context (List.nth_exn destructors n) [term t]
      | RecordUpdate _ -> adj ()
      | Cast (cbt, t) ->
...
*)
      ...
      and datatypeIsCons (c_nm, t) =
        ...
        let fd = List.Assoc.find_exn assocs ~equal:Sym.equal c_nm in
...
let pop solver n =
  ...
  trace (List.init n ~f:(fun _ -> Pop)) solver
...
module Eval = struct
  ...
  let eval global (context, model) to_be_evaluated =
    ...
    let rec func_interp func_decl =
      ...
      let loc = Locations.other __FUNCTION__ in
      List.fold_right ~f:(fun entry map_value ->
        ...
        ) entries ~init:(const_map_ (z3_sort argument_sort) base_value loc)
    ...
    and z3_expr (expr : Z3.Expr.expr) : IT.t =
      ...
      | () when Z3.Arithmetic.is_add expr ->
         List.fold_left ~f:(fun a b -> add_ (a, b) loc) ~init:(List.hd_exn args) (List.tl_exn args)
      ...
      | () ->
        ...
        | () when Z3Symbol_Table.mem z3sym_table func_name ->
           ...
           | MemberFunc {tag; member} ->
              ...
              let member_bt = Memory.bt_of_sct (List.Assoc.find_exn sd ~equal:Id.equal member) in
           ...
           | RecordMemberFunc {mbts; member} ->
              let member_bt = List.Assoc.find_exn mbts ~equal:Id.equal member in
