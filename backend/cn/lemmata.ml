...
let struct_layout_field_bts xs =
  ...
  let xs2 = List.filter ~f:(fun x -> Option.is_some x.member_or_padding) xs
    |> List.sort ~compare:(fun (x : struct_piece) y -> Int.compare x.offset y.offset)
...
let monomorphise_dt_lists global =
  ...
  let global = List.fold_right ~f:add_list_mono_datatype list_mono ~init:global in
...
let it_adjust (global : Global.t) it =
  let rec f t =
    ...
    match IT.term t with
        | IT.Binop (And, x1, x2) ->
            let xs = List.map ~f [x1; x2] |> List.partition_tf ~f:IT.is_true |> snd in
            IT.and_ xs loc
        | IT.Binop (Or, x1, x2) ->
            let xs = List.map ~f [x1; x2] |> List.partition_tf ~f:IT.is_false |> snd in
            IT.or_ xs loc
        | IT.Binop (EQ, x, y) ->
            let x = f x in
            let y = f y in
...
let tuple_element t (i, len) =
  ...
  let lhs = string "'" ^^ tuple_syn (List.init len ~f:nm) in
...
let tuple_upd_element t (i, len) y =
  ...
  let nm i = string ("x_t_" ^ Int.to_string i) in
  let lhs = string "'" ^^ tuple_syn (List.init len ~f:nm) in
  let rhs = tuple_syn (List.init len ~f:(fun j -> if j = i then y else nm j)) in
...
let ensure_tuple_op is_upd nm (ix, l) =
  ...
  let doc = lazy (
    ...
    let t_ty = tuple_coq_ty (!^ op_nm) (List.init l ~f:ty) in
    let t = parens (typ (!^ "t") t_ty) in
    let x = parens (typ (!^ "x") (ty ix)) in
    let infer = !^"{" ^^ flow (!^ " ") (List.init l ~f:ty) ^^ colon ^^^ !^"Type}" in
...
let ensure_pred global list_mono loc name aux =
  ...
  | Uninterp -> gen_ensure 1 ["params"; "pred"; Sym.pp_string name]
    (lazy (
      ...
      let ty = List.fold_right ~f:(fun at rt -> at ^^^ !^ "->" ^^^ rt) arg_tys ~init:ret_ty in
...
let generate (global : Global.t) directions (lemmata : (Sym.t * (Loc.t * AT.lemmat)) list) =
  ...
  let scan_lemmata =
    ...
    |> List.sort ~compare:(fun x (y : scanned) -> cmp_loc_line_numbers x.loc y.loc)
  in
