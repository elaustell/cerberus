...
module IndexTerms = struct
  ...
  let simp_int_comp a b loc =
      ...
      let repeated = List.sort ~compare:IT.compare (a_elems @ b_elems)
          |> group IT.equal
      ...
      let xs = List.sort ~compare:(fun a b -> IT.compare (fst a) (fst b)) (a_xs_neg @ b_xs)
          |> group (fun a b -> IT.equal (fst a) (fst b))
          |> List.map ~f:(fun xs -> (fst (List.hd_exn xs), List.fold_left ~f:Z.add ~init:Z.zero (List.map ~f:snd xs)))
      ...
      in
      (a_r, List.fold_left ~f:add_x ~init:b_r xs)
  ...
  let rec record_member_reduce it member =
    let loc = IT.loc it in
    match IT.term it with
      | Record members -> List.Assoc.find_exn members ~equal:Id.equal member
      | RecordUpdate ((t, m), v) ->
      ...
      | _ ->
        ...
        let member_bt = List.Assoc.find_exn member_tys ~equal:Id.equal member in
  ...
  let rec simp ?(inline_functions=false) simp_ctxt =
    ...
    | IT.StructMember (s_it, member) ->
       ...
       let rec make t =
         ...
         | IT (Struct (_, members), _, _) ->
            List.Assoc.find_exn members ~equal:Id.equal member
    ...
    | Constructor (ctor, ts) ->
       IT (Constructor (ctor, List.map ~f:(Pair.map_snd ~f:aux) ts), the_bt, the_loc)
    | WrapI (ity, t) ->
       let t = aux t in
