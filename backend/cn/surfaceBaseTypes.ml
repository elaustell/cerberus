...
let rec of_basetype = function
  ...
  | BT.Record member_types -> Record (List.map ~f:(Pair.map_snd ~f:of_basetype) member_types)
...
let rec to_basetype = function
  ...
  | Record member_types -> BT.Record (List.map ~f:(Pair.map_snd ~f:to_basetype) member_types)
