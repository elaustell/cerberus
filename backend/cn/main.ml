...
let frontend macros incl_dirs incl_files astprints do_peval filename state_file magic_comment_char_dollar =
  let open CF in
...
let main
  ...
  let (prog4, (markers_env, ail_prog), statement_locs) =
    handle_frontend_error
      (frontend macros incl_dirs incl_files astprints use_peval filename state_file magic_comment_char_dollar)
  in
