...
let mDefines defs t = List.fold_right ~f:mDefine defs ~init:t
let mResources ress t = List.fold_right ~f:mResource ress ~init:t
let mConstraints cons t = List.fold_right ~f:mConstraint cons ~init:t
