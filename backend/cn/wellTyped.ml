...
let correct_members_sorted_annotated loc spec have =
  ...
  let have = List.sort ~compare:compare_by_fst_id have in
  let have_annotated =
  ...
  let is_bt loc =
    let rec aux = function
      ...
      | Record members ->
         ...
         in
         assert (List.is_sorted_strictly ~compare:compare_by_fst_id members);
  ...
  let cases_necessary pats =
    ...
    let rec covers p1 p2 = match p1, p2 with
      ...
      | Pat (PConstructor (s1, ps1), _, _), Pat (PConstructor (s2, ps2), _, _)
        when (Sym.equal s1 s2 && List.equal Id.equal (List.map ~f:fst ps1) (List.map ~f:fst ps2)) ->
    ...
    let@ result = match it with
      ...
      | Struct (tag, members) ->
         ...
         let@ members_sorted =
           ListM.mapM (fun (id, ct) ->
               let@ t = check loc (Memory.bt_of_sct ct) (List.Assoc.find_exn members ~equal:Id.equal id) in
               return (id, t)
             ) decl_members
         in
      ...
      | RecordMember (t, member) ->
         ...
         let@ members = match IT.bt t with
           ...
           | has ->
             ...
             fail (illtyped_index_term loc t has ~expected ~reason)
         in
         let@ bt = match List.Assoc.find members ~equal:Id.equal member with
           | Some bt -> return bt
           | None ->
      ...
      | RecordUpdate ((t, member), v) ->
       ...
       | Cons (t1,t2) ->
          ...
          return (List.fold_right ~f:cons ((t1, loc) :: rest) ~init:last)
       | Head t ->
          let@ t = infer t in
...
let integer_annot annots =
  match List.dedup_and_sort ~compare:CF.IntegerType.setElemCompare_integerType (List.filter_map ~f:is_integer_annot annots) with
  | [] -> None
  | [ity] -> Some ity
  | _ -> assert false
...
module WDT = struct
      ...
      ListM.mapM (fun (c, args) ->
          let@ args =
            ListM.mapM (fun (id,bt) ->
                let@ bt = WBT.is_bt loc bt in
                return (id, bt)
              ) (List.sort ~compare:compare_by_fst_id args)
          in
          return (c, args)
        ) cases
    in
  ...
  let check_recursion_ok datatypes =
    ...
    let graph =
      List.fold_left ~f:(fun graph (dt, _) ->
          G.add_vertex graph dt
        ) ~init:graph datatypes
    in

    let graph =
      List.fold_left ~f:(fun graph (dt, dt_def) ->
          List.fold_left ~f:(fun graph dt' ->
              G.add_edge graph dt dt'
            ) ~init:graph (dts_in_dt_definition dt_def)
        ) ~init:graph datatypes
    in
    ...
    let@ () =
      ListM.iterM (fun scc ->
          ...
          ListM.iterM (fun dt ->
              let {loc;cases} = List.Assoc.find_exn datatypes ~equal:Sym.equal dt in
